...
/**
 * GitRepo represents the configuration of a Git repository.
 */
type GitRepo = {
...
/**
 * GitRepos represents a map of Git repositories, indexed by directory.
 */
type GitRepos = {
    [dir: string]: GitRepo;
};
...
type Factory = {
    [typeName: string]: {
        ...
        GetRepoInfo(dir: string): GitRepo;
    }
}
...
let factory: Factory = {
    ".git": {
        ...
        GetRepoInfo(dir): gitRepo {
            let gitRepo: GitRepo = {
                desc: fs.existsSync(dir + '/.git/description') ? fs.readFileSync(dir + '/.git/description', 'utf8').trim() : undefined,
                remotes: []
            };

            // 读取.git/config文件，获取远程仓库列表
            if (fs.existsSync(dir + '/.git/config')) {
                let config = fs.readFileSync(dir + '/.git/config', 'utf8').trim();
            }
